...
UmiGoalKeeper::UmiGoalKeeper (String^  _name):
  blocked           (false),
  blockerDepth      (0),
  blockerThreadId   (-1),
  name              (_name),
  ...
  while  (weAreBlocked)
  {
    ...
    catch  (Exception^ e)
    {
      ...
      if  (milliSecsBlocked > 10000)
        throw gcnew Exception ("UmiGoalKeeper::StartBlock    Name[" + name + "]  Could not establish a lock\n\n" + e->ToString ());
      weAreBlocked = true;
    }
...
void   UmiGoalKeeper::EndBlock ()
{
  ...
  {
    throw gcnew Exception ("UmiGoalKeeper::EndBlock    Name[" + name + "]  Exception trying to establish lock.\n\n" + e->ToString ());
  }
