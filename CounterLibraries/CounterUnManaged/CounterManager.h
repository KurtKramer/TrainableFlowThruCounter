 ...
 *@brief  This namespace contains the code that is specific to the Counter application.
 *@details  This namespace is meant to be a library that is O/S neutral; that is it has no O/S specific 
 * code.  It has all the data structures necessary to support the Counter.  The main class
 * in this routine is 'CounterManager'; all communication to and from this library should be
...
{
  /**
   *@brief  This is the main class used to manage the counter application.
   ...
   * 'GetStats' which utilizes an instance of 'CounterStats' to pass back current Counter Status.
   */
  class  CounterManager: public  KKObservable
  {
  public:
    ...
    enum  class CounterOperatingModes
                  {Null,
                   User,
                   Advanced,
                   Invalid
    ...
    static 
    const KKStr&  CounterOperatingModeToStr (CounterOperatingModes om);

    static
    CounterOperatingModes  CounterOperatingModeFromStr (const KKStr&  s);


    CounterManager (MsgQueuePtr  _msgQueue,
     ...
     *@details  Performs setup tasks such as loading CounterManager configuration file; makes 
     * initial attempt to connect to camera, etc...
     */
    void  Initialize ();

    KKStr  CounterVersionNumber ();

    CounterState  CurState ()  {return curState;}

    CounterOperatingModes  OperatingMode ()  {return   operatingMode;}
     ...
     *@details  This method will load the configuration file specified by '_trainingModelName' and retrieve Counter specific
    ...
    const KKStr&  DropFolderToRemote             () const {return dropFolderToRemote;}
    ...
    const CounterStatsPtr  GetFinaleStats () {return finaleStats;}
    ...
    void  GetStats (CounterStats&  stats);
  ...
  private:
    ...
    void  ValidateCounterInstallation ();
    ...
    CounterOperatingModes         operatingMode;
    MsgQueue                      secondaryMsgs;           /**< Add messages that are to be displayed to the screen  on next call to 'Status' */
    KKStr                         dropFolderToRemote;      /**< Root directory where files are deposited that need to be transmitted to remote.
    ...
    InstallationConfigPtr         installation;            /**< Represents the physical parameters of the installation that we are recording on or
                                                            * playing back from.   It will initially be created when 'CounterManager' is 
    ...
    CounterStatsPtr               finaleStats;             /**< Stat's at time of last PlayBack or recording session.  */
    ...
    MsgQueuePtr                   loggedMsgs;             /**< Messages that have been written to the log file in 'LoggerThread' are copied to this MsgQueue and are removed by owner of 'CounterManager' when needed for display. */
