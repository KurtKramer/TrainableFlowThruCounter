...
namespace CounterManaged
{
  ...
  private:
    ...
    static  CounterManaged::UmiGoalKeeper^  getUniqueClassBlocker      // Will make sure that two threads don't try to
      = gcnew CounterManaged::UmiGoalKeeper ("UniqueClassBlocker");    // add the same class name at the same time.
