...
bool ImageSaveDlg::SaveImage( PvBuffer* aBuffer )
{
    ...
    PvUInt32 lBitsPerPixel = ( mFormat == FORMAT_BMP ) ? 32 : aBuffer->GetBitsPerPixel();    

    // 1048576 bits per Mbit
    PvUInt32 lBitsPerImage = aBuffer->GetWidth() * aBuffer->GetHeight() * lBitsPerPixel;
    mMbps = static_cast<double>( lBitsPerImage ) * mFPS / 1048576.0;
    mTotalSize += ( lBitsPerImage / 8 );
...
bool ImageSaveDlg::SaveIfNecessary( PvBuffer *aRawBuffer )
{
    ...
    PvUInt32 lBitsPerPixel = ( mFormat == FORMAT_BMP ) ? 32 : aRawBuffer->GetBitsPerPixel();    
    ...
    case AVERAGETHROUGHPUT:
        ...
        lBitsPerImage = aRawBuffer->GetWidth() * aRawBuffer->GetHeight() * lBitsPerPixel;
